...
const Card = ({ header, description, stack, link }: IProps) => {
  ...
  return (
    <div
      className={`${style.cardshadow} card card-side rounded-none border-x border-black bg-slate-800  hover:-translate-y-3 hover:-translate-x-3 transition duration-200 ease-in-out`}
    >
      <div className="card-body gap-8">
        <h2
          className="card-title justify-center bg-gradient-to-tr from-yellow-500 via-yellow-200 to-yellow-500
           bg-clip-text text-3xl font-bold  text-transparent"
        >
          {header}
        </h2>
        <p>{description}</p>
        <div className="card-actions justify-center column">
